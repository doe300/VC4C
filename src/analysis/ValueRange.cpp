...
ValueRange::ValueRange(bool isFloat, bool isSigned) :
    type(isFloat ? RangeType::FLOAT : RangeType::INTEGER), hasDefaultBoundaries(true)
{
...
ValueRange::ValueRange(const DataType& type) : ValueRange(type.isFloatingType(), !isUnsignedType(type))
{
    ...
    {
        intRange.minValue = 0;
        intRange.maxValue = elemType.getScalarBitCount() == 8 ?
    ...
    {
        intRange.minValue = elemType.getScalarBitCount() == 8 ?
...
FastMap<const Local*, ValueRange> ValueRange::determineValueRanges(Method& method)
{
    ...
    while(!it.isEndOfMethod())
    {
        if(it.has() && !it.has<BranchLabel>() && it->hasValueType(ValueType::LOCAL))
        {
            ...
            if(it->hasDecoration(InstructionDecorations::BUILTIN_GLOBAL_ID) ||
            ...
            else if(it->hasDecoration(InstructionDecorations::BUILTIN_LOCAL_ID))
            {
                ...
                {
                    maxID = *std::max_element(
                                method.metaData.workGroupSizes.begin(), method.metaData.workGroupSizes.end()) -
                        1;
                }
                else
                    maxID = 11;
            ...
            else if(constant.ifPresent(toFunction(&Value::isLiteralValue)))
            {
                if(constant->type.isFloatingType())
                    ...
                    range.extendBoundaries(std::min(static_cast<int64_t>(constant->getLiteralValue()->signedInt()),
            ...
            // could change afterwards!)
                ...
                (op->op == OP_ADD || op->op == OP_AND || op->op == OP_FADD || op->op == OP_FMAX ||
                    op->op == OP_FMAXABS || op->op == OP_FMIN || op->op == OP_FMINABS || op->op == OP_FMUL ||
                    op->op == OP_FSUB || op->op == OP_ITOF || op->op == OP_MAX || op->op == OP_MIN ||
                    op->op == OP_MUL24 || op->op == OP_SHR || op->op == OP_SUB) &&
                std::all_of(it->getArguments().begin(), it->getArguments().end(), [](const Value& arg) -> bool {
                    return arg.isLiteralValue() || (arg.getSingleWriter() != nullptr);
                }))
            {
                /*
                ...
                if(it->getArguments().size() > 1)
                {
                    ...
                    if(arg1.isLiteralValue())
                    {
                        if(arg1.type.isFloatingType())
                            secondRange.extendBoundaries(
                                arg1.getLiteralValue()->real(), arg1.getLiteralValue()->real());
                        else
                            secondRange.extendBoundaries(
                ...
                if(it->hasDecoration(InstructionDecorations::UNSIGNED_RESULT) &&
                    !it->getOutput()->type.isFloatingType())
                {
                ...
                if(minVal.ifPresent(toFunction(&Value::isLiteralValue)) &&
                    maxVal.ifPresent(toFunction(&Value::isLiteralValue)))
                {
                    if(it->getOutput()->type.isFloatingType())
                        range.extendBoundaries(minVal->getLiteralValue()->real(), maxVal->getLiteralValue()->real());
                    else
                        range.extendBoundaries(std::min(static_cast<int64_t>(minVal->getLiteralValue()->signedInt()),
                    ...
                    range.extendBoundariesToUnknown(isUnsignedType(it->getOutput()->type) ||
                        it->hasDecoration(InstructionDecorations::UNSIGNED_RESULT));
            }
            else
            {
