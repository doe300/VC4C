...
namespace vc4c
{
	namespace periphery
	{
		 ...
		 * The VC4 has 2 TMUs shared by all QPUs on a slice. Nevertheless, querying memory via a TMU does not require a mutex-lock.
		 * The TMU memory-lookup is per-element, so element X will return the value from the address  given by element X, write 0 per element to disable.
		 */

		/*
		 * Perform a general memory lookup via the TMU.
		 *
		 * Reads the required number of 32-bit vectors from the given address via the TMU and performs the necessary type conversions and vector rotations (for non 32-bit types).
		 */
		InstructionWalker insertReadVectorFromTMU(Method& method, InstructionWalker it, const Value& dest, const Value& addr, const TMU& tmu = TMU0);
		 ...
		 */
		InstructionWalker insertGeneralReadTMU(Method& method, InstructionWalker it, const Value& dest, const Value& addr, const TMU& tmu = TMU0);
		 ...
		 */
		InstructionWalker insertReadTMU(Method& method, InstructionWalker it, const Value& image, const Value& dest, const Value& xCoord, const Optional<Value>& yCoord = NO_VALUE, const TMU& tmu = TMU0);
