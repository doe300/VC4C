...
TemporaryFile::TemporaryFile(const std::string& fileTemplate) : fileName(fileTemplate)
{
    ...
    if(fchmod(fd, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH) < 0)
        throw CompilationError(
            CompilationStep::PRECOMPILATION, "Failed to change the access-mode for temporary file", strerror(errno));
#endif
...
static std::string buildCommand(const std::string& compiler, const std::string& defaultOptions,
...
static void runPrecompiler(const std::string& command, std::istream* inputStream, std::ostream* outputStream,
    const Optional<std::string>& tempFile)
{
    ...
    const bool toText = true, const Optional<std::string>& inputFile = {}, const Optional<std::string>& outputFile = {})
{
    ...
    // emit LLVM IR
    const std::string command = buildCommand(compiler, defaultOptions, options,
...
static void compileLLVMIRToSPIRV(std::istream& input, std::ostream& output, const std::string& options,
