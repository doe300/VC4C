...
{
	/*
	 * We use relative targets, since the absolute address is completely absolute and not relative to the start of the code-block.
	 *  Since we do not know the start of the code-block (where the QPU code is loaded to), we use relative addresses.
	 *
	...
	auto qasm = new qpu_asm::BranchInstruction(cond, BranchRel::BRANCH_RELATIVE, BranchReg::NONE, 0 /* only 5 bits, so REG_NOP doesn't fit */, REG_NOP.num, REG_NOP.num, static_cast<int32_t>(branchOffset), "to " + this->getTarget()->name);
	return qasm;
}
...
qpu_asm::Instruction* PhiNode::convertToAsm(const FastMap<const Local*, Register>& registerMapping, const FastMap<const Local*, std::size_t>& labelMapping, const std::size_t instructionIndex) const
{
	throw CompilationError(CompilationStep::CODE_GENERATION, "There should be no more phi-nodes at this point", to_string());
}
