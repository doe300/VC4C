...
static void printHelp()
{
	...
	std::cout << "\t--kernel-info\t\tWrite the kernel-info meta-data (as required by VC4CL run-time, default)" << std::endl;
...
int main(int argc, char** argv)
{
    ...
    if(inputFiles.size() > 1)
    {
    	...
    	{
    		std::cerr << "Selected front-end does not support the input-format generated by the linker, aborting! " << std::endl;
    		return 5;
    	}
    }
    else
