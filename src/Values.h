...
namespace vc4c
{
	...
	struct Register
	{
		...
		/*
		 * Whether writing into this register triggers the result to appear in r4, e.g. by triggering an SFU calculation or loading via TMU
		 */
	...
	/*
	 * UNIFORM registers, present on both physical register-files, used to read the kernel parameters and image-configuration
	 */
	...
	/*
	 * Writing to "accumulator 4" toggles the automatic swapping of TMUs (using TMU0 for QPUs 0/1 and TMU1 for QPUs 2/3 within a slice).
	 * Writing any value different from zero disables the TMU swapping and allows all QPUs to access both TMUs by writing in their corresponding registers.
	 */
	...
	/*
	 * Writing to this register distributes the value from the first element of the quad (0, 4, 8, 12) to all other elements of this quad
	 */
...
//Special Functions Unit
	...
	/*
	 * Writing this register triggers the SFU to approximate the logarithm to the power of two (log2(x)) of the floating-point value passed.
	 * The result is available in r4 three instructions later, in which no SFU-register nor r4 can be touched!
	 */
...
//if we leave TMU auto-swap enabled, we only need to write to TMU0, otherwise, we need to access both TMUs
	 ...
	 *
	 * Depending on whether the T-coordinate was written before, this load is a texture load (if written) or a general 32-bit load (otherwise).
	 *
	 * NOTE: Loads from TMU are element-wise, so with a single load, 16 values from 16 different memory-locations can be read.
	...
	/*
	 * Writing this register sets the border color to be used (depending on the clamp-mode) for out-of-range coordinate-reads
	 */
	 ...
	 *
	 * SmallImmediates are no "standard" integer values, but are mapped to represent some value (e.g. using a SmallImmediate value of 16 actually loads the integer value -15).
	 * Additionally, some SmallImmediate values are used to load floating-point constants as well as represent the offset for vector-rotations.
	 *
	...
	struct SmallImmediate : public InstructionPart
	{
		 ...
		 *
		 * The field-value of VECTOR_ROTATE_R5 represents a vector-rotation by the value stored in SIMD-element 0 of r5, bits [3:0],
		 * while the field-values 49 - 63 represent vector-rotations by an offset of 1 - 15 upwards (so element 0 moves to element 1 - 15)
		 *
	...
	struct Value
	{
		 ...
		 *
		 * This function also accepts, if this object contains a SmallImmediate with the same integer- or floating-point-value (depending on the data-type) as the parameter
		 */
		bool hasLiteral(const Literal& lit) const;
		/*
		 * Whether this object contains the given SmallImmediate value.
		 *
		 * This function also accepts, if this object contains a Literal with the same integer- or floating-point-value (depending on the data-type) as the parameter
		 */
