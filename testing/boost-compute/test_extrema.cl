...
//manual test correct
__kernel void find_extrema_min_max(__global int* _buf0, __global uint* index)
{
...
__kernel void find_extrema_on_cpu_min_max(uint count, __global int* output, __global uint* output_idx, __global int* _buf0)
{
...
__kernel void serial_find_extrema_min_max(__global int* _buf0, __global uint* index, uint size)
{
int value = _buf0[0];
uint value_index = 0;
...
//TODO fix and add to emulation tests
__kernel void find_extrema_reduce_min_max(uint count, __local int* block, __local uint* block_idx, __global uint* _buf0, __global int* _buf1, __global int* _buf2)
{
const uint gid = get_global_id(0);
uint idx = gid;
int acc;
