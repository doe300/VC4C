...
MethodCall::MethodCall(const std::string& methodName, const std::vector<Value>& args) : IntermediateInstruction(NO_VALUE), methodName(methodName)
{
...
qpu_asm::Instruction* MethodCall::convertToAsm(const FastMap<const Local*, Register>& registerMapping, const FastMap<const Local*, std::size_t>& labelMapping, const std::size_t instructionIndex) const
{
...
{
    return std::string("return ") + (getReturnValue() ? getReturnValue().to_string() : "") + createAdditionalInfoString();
}
...
qpu_asm::Instruction* Return::convertToAsm(const FastMap<const Local*, Register>& registerMapping, const FastMap<const Local*, std::size_t>& labelMapping, const std::size_t instructionIndex) const
{
    throw CompilationError(CompilationStep::LABEL_REGISTER_MAPPING, "There should be no more returns at this point", to_string());
}
