...
static ParameterDecorations parseParameterDecorations(Scanner& scanner)
{
    ...
    while(
        scanner.peek().hasValue("readonly") || scanner.peek().hasValue("zeroext") || scanner.peek().hasValue("signext"))
    {
...
DataType IRParser::parseType()
{
    ...
    while(true)
    {
        // if the pointed to type is a pointer with an address space, the pointer can have multiple address space
        // declarations  e.g. "%struct.Node.0 addrspace(1)* addrspace(1)* %pNext"
...
std::vector<std::pair<Value, ParameterDecorations>> IRParser::parseParameters()
{
    ...
    do
    {
        ...
        else
        {
            ...
            ParameterDecorations decorations = ParameterDecorations::NONE;
            while(scanner.hasInput() && scanner.peek().type == TokenType::STRING &&
                !(scanner.peek().hasValue(',') || scanner.peek().hasValue('<') || scanner.peek().hasValue('%')))
            {
                // reads attributes
            ...
            if(!scanner.peek().isEnd() &&
                (scanner.peek().hasValue('<') || scanner.peek().hasValue('%') ||
                    scanner.peek().type != TokenType::STRING))
            {
            ...
            else
            {
                ...
                if(nextToken.type == TokenType::STRING && !nextToken.hasValue('%'))
                {
                    ...
                    strncpy(nextToken.text.data(), (std::string("%") + std::to_string(res.size())).data(),
                        TOKEN_BUFFER_SIZE);
                }
...
static std::vector<Value> parseStringConstant(const std::string& constant, const std::size_t numEntries)
{
    ...
    // example string:
    // c"R\09j\D506\A58\BF@\A3\9E\81\F3\D7\FB|\E39\82\9B/\FF\874\8ECD\C4\DE\E9\CBT{\942\A6\C2#=\EEL\95\0BB\FA\C3N\08.\A1f(\D9$\B2v[\A2Im\8B\D1%r\F8\F6d\86h\98\16\D4\A4\5C\CC]e\B6\92lpHP\FD\ED\B9\DA^\15FW\A7\8D\9D\84\90\D8\AB\00\8C\BC\D3\0A\F7\E4X\05\B8\B3E\06\D0,\1E\8F\CA?\0F\02\C1\AF\BD\03\01\13\8Ak:\91\11AOg\DC\EA\97\F2\CF\CE\F0\B4\E6s\96\ACt\22\E7\AD5\85\E2\F97\E8\1Cu\DFnG\F1\1Aq\1D)\C5\89o\B7b\0E\AA\18\BE\1B\FCV>K\C6\D2y
    // \9A\DB\C0\FEx\CDZ\F4\1F\DD\A83\88\07\C71\B1\12\10Y'\80\EC_`Q\7F\A9\19\B5J\0D-\E5z\9F\93\C9\9C\EF\A0\E0;M\AE*\F5\B0\C8\EB\BB<\83S\99a\17+\04~\BAw\D6&\E1i\14cU!\0C}"
    // character values: 0x52U, 0x09U, 0x6aU, 0xd5U, 0x30U, 0x36U, 0xa5U, 0x38U, 0xbfU, 0x40U, 0xa3U, 0x9eU, 0x81U,
    // 0xf3U, 0xd7U, 0xfbU,	0x7cU, 0xe3U, 0x39U, 0x82U, 0x9bU, 0x2fU, 0xffU, 0x87U,	0x34U, 0x8eU, 0x43U, 0x44U, 0xc4U,
    // 0xdeU, 0xe9U, 0xcbU, ...
...
Value IRParser::parseValue(bool withType, const DataType& typeArg)
{
    ...
    if(nextToken.hasValue('<') || nextToken.hasValue('[') || nextToken.hasValue('{'))
    {
        ...
        while(!scanner.peek().hasValue('>') && !scanner.peek().hasValue(']') && !scanner.peek().hasValue('}'))
        {
            ...
            // e.g. "@grads3d = addrspace(2) constant [16 x <4 x i8>] [<4 x i8> <i8 1, i8 1, i8 0, i8 0>, <4 x i8> <i8
            // -1, i8 1, i8 0, i8 0>, ...]"
...
DataType IRParser::parseStructDefinition()
{
    ...
    logging::debug() << "Struct type: " << type.to_string()
                     << (type.getStructType().value()->isPacked ? " (packed)" : "") << logging::endl;
    if(!type.getStructType().value()->elementTypes.empty())
        logging::debug() << "with elements: " << to_string<DataType>(type.getStructType().value()->elementTypes)
                         << logging::endl;
    return type;
}
...
static void skipLinkage(Scanner& scanner)
{
    ...
    while(scanner.peek().hasValue("private") || scanner.peek().hasValue("internal") ||
        scanner.peek().hasValue("available_externally") || scanner.peek().hasValue("linkonce") ||
        scanner.peek().hasValue("weak") || scanner.peek().hasValue("common") || scanner.peek().hasValue("appending") ||
        scanner.peek().hasValue("extern_weak") || scanner.peek().hasValue("external") ||
        scanner.peek().hasValue("linkonce_odr") || scanner.peek().hasValue("weak_odr"))
    {
        scanner.pop();
    }
}
...
{
    while(
        scanner.peek().hasValue("default") || scanner.peek().hasValue("hidden") || scanner.peek().hasValue("protected"))
    {
        scanner.pop();
    }
}
...
bool IRParser::parseGlobalData()
{
    ...
    {
        const DataType type(parseType());
        // TODO global as bitcast of address of of other global (e.g. in
        // /opt/SPIRV-LLVM/tools/clang/test/CodeGenOpenCL/opencl_types.cl). How to handle??  would need to retain in the
        // same address of the other global?
        if(scanner.peek().hasValue("bitcast"))
            throw CompilationError(
                CompilationStep::PARSER, "Global as bitcast to other global is not implemented yet", name);
...
{
    // define [linkage] [visibility] [DLLStorageClass] [cconv] [ret attrs] <ResultType> @<FunctionName> ([argument
    // list]) [(unnamed_addr|local_unnamed_addr)] [fn Attrs] [section "name"]
    bool isKernelSet = false;
    logging::debug() << "-----" << logging::endl;
    skipLinkage(scanner);
    skipVisibility(scanner);
    // skip keywords
    while(scanner.peek().hasValue("signext") || scanner.peek().hasValue("zeroext") ||
        scanner.peek().hasValue("spir_func") || scanner.peek().hasValue("spir_kernel"))
    {
    ...
    do
    {
        ...
        else if(nextToken.hasValue("!vec_type_hint"))
        {
            ...
            logging::info() << "Vector type hint is currently not supported: " << scanner.pop().getText().value()
                            << logging::endl;
        }
...
void IRParser::parseMethodBody(LLVMMethod& method)
{
     ...
     * SPIRV-LLVM/CLang adds an additional label to the beginning, so this introduces two labels with the same position
     * (an empty basic block), which has no negative effect (maybe a little loss in performance)
     */
...
void IRParser::parseInstruction(
    LLVMMethod& method, FastModificationList<std::unique_ptr<LLVMInstruction>>& instructions)
{
    ...
    else
    {
        ...
        throw CompilationError(CompilationStep::PARSER, scanner.getLineNumber(),
            std::string("Unknown instruction: ") + nextToken.to_string());
    }
    // skip to end of line
...
static intermediate::InstructionDecorations parseFastMathFlags(Scanner& scanner)
{
    ...
    while(scanner.peek().hasValue("nsw") || scanner.peek().hasValue("nuw") || scanner.peek().hasValue("nnan") ||
        scanner.peek().hasValue("ninf") || scanner.peek().hasValue("nsz") || scanner.peek().hasValue("arcp") ||
        scanner.peek().hasValue("fast") || scanner.peek().hasValue("exact"))
    {
    ...
    while(curIndex < indices.size())
    {
        ...
        else if(subContainerType.getStructType())
        {
            ...
            if(!idxVal.getLiteralValue())
                throw CompilationError(CompilationStep::LLVM_2_IR, "Cannot access struct-element with non-scalar index",
                    idxVal.to_string());
            elementTypes.push_back(
                subContainerType.getStructType().value()->elementTypes.at(idxVal.getLiteralValue()->unsignedInt()));
        }
        else if(subContainerType.isVectorType())
        {
            ...
            // Since the lifetime-start instruction always takes an i8*, for vectors of i8 elements, the pointer of the
            ...
            throw CompilationError(
                CompilationStep::LLVM_2_IR, "Cannot access index of type", subContainerType.to_string());
...
static std::string cleanMethodNameParameters(const std::string& name, const std::vector<Value>& args)
{
    ...
    while(it != args.rend())
    {
        if(it->type.isScalarType() || it->type.isVectorType())
        {
            ...
            else if((it->type.getElementType() == TYPE_INT32 || it->type.getElementType() == TYPE_INT16 ||
                        it->type.getElementType() == TYPE_INT8) &&
                (result.back() == 'i' || result.back() == 'j'))
                result = result.substr(0, result.size() - 1);
        }
        ++it;
    }
...
IndexOf* IRParser::parseGetElementPtr(LLVMMethod& method, const std::string& destination)
{
    ...
    logging::debug() << "Getting element " << to_string<Value>(indices) << " from " << pointer.to_string() << " into "
                     << dest.to_string(true) << logging::endl;
    return new IndexOf(dest, pointer, indices);
}

void IRParser::parseAssignment(
    LLVMMethod& method, FastModificationList<std::unique_ptr<LLVMInstruction>>& instructions, const Token& dest)
{
    ...
    if(nextToken.hasValue("alloca"))
    {
        ...
        logging::debug() << "Stack-allocation for " << type.to_string() << " " << destination << " with alignment of "
                         << alignment << " bytes" << logging::endl;
        method.method->stackAllocations.emplace(
            StackAllocation(destination, type.toPointerType(), type.getPhysicalWidth(), alignment));
        return;
    }
    else if(nextToken.hasValue("bitcast"))
    {
        ...
        logging::debug() << "Making reference from bitcast from " << src.to_string() << " to " << dest.to_string()
                         << logging::endl;
    ...
    else if(nextToken.hasValue("ptrtoint") || nextToken.hasValue("inttoptr"))
    {
        ...
        logging::debug() << "Casting between pointer and integer: " << src.to_string() << " to " << dest.to_string()
                         << logging::endl;
    ...
    else if(nextToken.hasValue("load"))
    {
         ...
         * !invariant.load !<index>][, !invariant.group !<index>][, !nonnull !<index>][, !dereferenceable
         * !<deref_bytes_node>][, !dereferenceable_or_null !<deref_bytes_node>][, !align !<align_node>] <result> = load
         * atomic [volatile] <ty>, <ty>* <pointer> [singlethread] <ordering>, align <alignment> [, !invariant.group
         * !<index>]
         */
        ...
        logging::debug() << "Copying by loading of " << type.to_string() << " from " << src.to_string() << " into "
                         << destination << logging::endl;
        ...
        if(src.hasType(ValueType::LOCAL) && src.local->is<Global>())
        {
            ...
            instructions.emplace_back(
                new ContainerExtraction(method.method->findOrCreateLocal(type, destination), src, INT_ZERO));
            return;
        }
        ...
        else
        {
            ...
            throw CompilationError(CompilationStep::PARSER, scanner.getLineNumber(),
                "Loading without 'getelementptr' is not implemented!");
        }
        // for (almost) any load from an array/pointer position, the previous instruction is a getelementptr, which gets
        // the index  only exception is the first load into ptr[0]
        if(srcContainer.hasType(ValueType::LOCAL))
        {
            if(srcContainer.local->is<Parameter>())
            {
                ...
                instructions.emplace_back(
                    new Copy(method.method->findOrCreateLocal(type, destination)->createReference(), src, true, true));
                return;
            }
        }
        instructions.emplace_back(
    ...
    else if(nextToken.hasValue("call") || nextToken.hasValue("tail") || nextToken.hasValue("notail") ||
        nextToken.hasValue("musttail"))
    {
        /*
         * <result> = [tail | musttail | notail ] call [fast-math flags] [cconv] [ret attrs] <ty>|<fnty>
         * <fnptrval>(<function args>) [fn attrs] [ operand bundles ]
         */
        // pop following 'call'
        ...
        std::for_each(params.begin(), params.end(),
            [&args](const std::pair<Value, ParameterDecorations>& pair) { args.push_back(pair.first); });
        name = cleanMethodNameParameters(name, args);
        logging::debug() << "Method call to " << name << " storing " << returnType.to_string() << " into "
                         << destination << logging::endl;
        method.method->findOrCreateLocal(returnType, destination);
        instructions.emplace_back(
            (new CallSite(method.method->findOrCreateLocal(returnType, destination), name, returnType, args))
    ...
    else if(nextToken.hasValue("icmp") || nextToken.hasValue("fcmp"))
    {
        ...
        logging::debug() << "Comparison " << flag << " between " << op1.to_string() << " and " << op2.to_string()
                         << " into " << destination << logging::endl;
        instructions.emplace_back((new Comparison(method.method->findOrCreateLocal(TYPE_BOOL, destination), flag, op1,
                                       op2, nextToken.hasValue("fcmp")))
                                      ->setDecorations(decorations));
        return;
    }
    else if(nextToken.hasValue("insertelement") || nextToken.hasValue("insertvalue"))
    {
        ...
        logging::debug() << "Setting container element " << index.to_string() << " of " << container.to_string()
                         << " to " << newValue.to_string() << logging::endl;
        instructions.emplace_back(new ContainerInsertion(
            method.method->findOrCreateLocal(container.type, destination), container, newValue, index));
        return;
    }
    else if(nextToken.hasValue("extractelement") || nextToken.hasValue("extractvalue"))
    {
        ...
        logging::debug() << "Reading container element " << index.to_string() << " of " << container.to_string()
                         << " into " << destination << logging::endl;
        instructions.emplace_back(new ContainerExtraction(
            method.method->findOrCreateLocal(container.type.getElementType(), destination), container, index));
        return;
    }
    ...
    else if(nextToken.hasValue("select"))
    {
        ...
        logging::debug() << "Selection of " << val1.to_string() << " or " << val2.to_string() << " according to "
                         << cond.to_string() << " into " << destination << logging::endl;
        instructions.emplace_back(
            new Selection(method.method->findOrCreateLocal(val1.type, destination), cond, val1, val2));
        return;
    }
    else
    {
        ...
        {
            throw CompilationError(CompilationStep::PARSER, scanner.getLineNumber(),
                std::string("Unhandled instruction: ") + nextToken.to_string());
        }
        decorations = add_flag(decorations, parseFastMathFlags(scanner));
        const DataType type(parseType());
        ...
        if(!nextToken.isEnd())
        {
            ...
            {
                const DataType destType = parseType();
                logging::debug() << "Convert (" << opCode << ") " << arg1.to_string() << " to " << destType.to_string()
                                 << ' ' << destination << logging::endl;
                instructions.emplace_back(
                    (new UnaryOperator(
                         opCode, method.method->findOrCreateLocal(destType, destination)->createReference(), arg1))
                        ->setDecorations(decorations));
            }
            else
            {
                const Value arg2 = parseValue(false, type);
                logging::debug() << "Binary-Operator " << opCode << " with " << arg1.to_string() << " and "
                                 << arg2.to_string() << " into " << destination << logging::endl;
                instructions.emplace_back(
                    (new BinaryOperator(
                         opCode, method.method->findOrCreateLocal(type, destination)->createReference(), arg1, arg2))
                        ->setDecorations(decorations));
            }
        }
        else
        {
            // unary instruction
            logging::debug() << "Unary-Operator " << opCode << " with " << type.to_string() << ' ' << arg1.to_string()
                             << " into " << destination << logging::endl;
            instructions.emplace_back((
                new UnaryOperator(opCode, method.method->findOrCreateLocal(type, destination)->createReference(), arg1))
                                          ->setDecorations(decorations));
        }
        return;
    }
...
{
    /*
     * store [volatile] <ty> <value>, <ty>* <pointer>[, align <alignment>][, !nontemporal !<index>][, !invariant.group
     * !<index>]        ; yields void store atomic [volatile] <ty> <value>, <ty>* <pointer> [singlethread] <ordering>,
     * align <alignment> [, !invariant.group !<index>] ; yields void
     */
    skipToken(scanner, "atomic");
    // XXX handle volatile?
    skipToken(scanner, "volatile");
    ...
    logging::debug() << "Copying by storing " << value.to_string() << " into " << destination.to_string()
                     << logging::endl;
    ...
    else
    {
        ...
        {
            throw CompilationError(
                CompilationStep::PARSER, scanner.getLineNumber(), "This type of storing is not implemented!");
        }
    }
    // for (almost) any store to an array/pointer position, the previous instruction is a getelementptr, which sets the
    // index  only exception is the first store into ptr[0]
    if(destContainer.hasType(ValueType::LOCAL))
    {
        // check whether output is a parameter
        // FIXME rewrite
        //    	throw CompilationError(CompilationStep::PARSER, "This type of storing is not implemented yet",
        //    destContainer.to_string());
        ...
        //            if (tmp && tmp.get().value.hasType(ValueType::LOCAL) &&
        //            method.method.findParameter(tmp.get().value.local)) {
...
void IRParser::parseBranch(LLVMMethod& method, FastModificationList<std::unique_ptr<LLVMInstruction>>& instructions)
{
    ...
    else
    {
        ...
        logging::debug() << "Branch on " << cond.to_string() << " to either " << trueLabel << " or " << falseLabel
                         << logging::endl;
        instructions.emplace_back(new Branch(cond, method.method->findOrCreateLocal(TYPE_LABEL, trueLabel),
            method.method->findOrCreateLocal(TYPE_LABEL, falseLabel)));
    }
}
...
void IRParser::parseLabel(
    LLVMMethod& method, FastModificationList<std::unique_ptr<LLVMInstruction>>& instructions, const Token& label)
{
...
void IRParser::parseSwitch(LLVMMethod& method, FastModificationList<std::unique_ptr<LLVMInstruction>>& instructions)
{
    ...
    logging::debug() << "Switching on " << cond.to_string() << " with " << cases.size() << " labels, defaulting to "
                     << defaultLabel << logging::endl;
...
void IRParser::parseMetaData()
{
    ...
    expectSkipToken(scanner, '!');
    Token nextToken{};
    while(scanner.hasInput() && !(nextToken = scanner.pop()).isEnd() &&
        (nextToken.hasValue(',') || nextToken.hasValue('{')))
    {
...
void IRParser::extractKernelInfo()
{
    ...
    {
        kernelIDs.insert(
            kernelIDs.begin(), metaData.at("!opencl.kernels").begin(), metaData.at("!opencl.kernels").end());
    }
        ...
        if(pair.second.size() > 0)
        {
            ...
            else if(pair.second[0] == "vec_type_hint")
            {
                ...
                logging::info() << "Vector type hint is currently not supported: "
                                << to_string<std::string>(pair.second) << logging::endl;
            }
        }
    }
    ...
    for(LLVMMethod& method : methods)
    {
        for(const auto& pair : method.metaDataMapping)
        {
            ...
            case MetaDataType::WORK_GROUP_SIZES:
                method.method->metaData.workGroupSizes = {static_cast<uint32_t>(std::atoi(values.at(0).data())),
                    static_cast<uint32_t>(std::atoi(values.at(1).data())),
                    static_cast<uint32_t>(std::atoi(values.at(2).data()))};
                break;
            case MetaDataType::WORK_GROUP_SIZES_HINT:
                method.method->metaData.workGroupSizeHints = {static_cast<uint32_t>(std::atoi(values.at(0).data())),
                    static_cast<uint32_t>(std::atoi(values.at(1).data())),
                    static_cast<uint32_t>(std::atoi(values.at(2).data()))};
                break;
            default:
...
void IRParser::mapInstructions(LLVMMethod& method) const
{
    ...
    logging::debug() << "Done, generated " << method.method->countInstructions() << " immediate instructions from "
                     << method.instructions.size() << " LLVM instructions" << logging::endl;
}
